...
__all__ = ['KlothedBodyV4_1']

log = logging.getLogger(__name__)

class KlothedBodyV4_1(Callable):
    ...
    def __init__(self,
        ...
        has_decomposed_betas:       bool=True,
        gender:                     str='neutral',
    ...
    def calculate_ipd_based_height(self,
        ...
        ''' Erica 4:44 PM
            Confirmed with the head of the bloomingdales studio…
            Men 6ft- 6’2, Women 5’9- 5’10
        '''
        stats = {            
            'male': { 
                'avg_ipd': 6.4, 
        ...
        stats.update({'neutral': { 
                'avg_ipd': (stats['male']['avg_ipd'] + stats['female']['avg_ipd']) * 0.5, 
                'std_ipd': (stats['male']['std_ipd'] + stats['female']['std_ipd']) * 0.5, 
                'min_height': stats['female']['min_height'],
                'max_height': stats['male']['max_height'],
        }})
        # men_avg, men_std = 6.4, 0.34
        # women_avg, women_std = 6.17, 0.36
        mesh_height = data['height'].detach().cpu()
        height = data['height'].detach().cpu() * (stats[gender]['avg_ipd'] / ipd) # in meters
        if gender in self.height_regressors:
            data = np.array([
                float(mesh_height), 
                float(ipd * 0.01), 
                float(ipd * 0.01 / mesh_height),
                float(ipd * 0.01 / stats[gender]['avg_ipd'])
            ])
            raw_height = float(self.height_regressors[gender].predict(data[np.newaxis, :]))
        else:
            raw_height = height
        actual_height = np.clip(raw_height, stats[gender]['min_height'], stats[gender]['max_height'])
        ...
        return (
            ...
            torch.scalar_tensor(float(raw_height)),
        )
    ...
    def save_body_legacy(self, 
        ...
        for i in range(len(jsons)):
            ...
            gender = 'neutral' if gender_id is None else toolz.get(int(gender_id[i]), KlothedBodyV4_1._GENDERS_, 'neutral')
            ipd, estimated_height, mesh_height, actual_height, raw_height = \
                self.calculate_ipd_based_height(data, gender)
            with open(jsons[i]['body']['body_legacy_t'], 'wb') as output_file:
                pickle.dump({                    
                    ...
                    'raw_height': raw_height[np.newaxis, ...].detach().cpu().numpy(),
                    'version': KlothedBodyV4_1.__VERSION__,
                }, output_file)
    ...
    def __call__(self, 
        ...
        for i in range(b):
            has_no_metadata = 'metadata_t' not in jsons[i]['body'] or jsons[i]['body']['metadata_t'] is None or not os.path.exists(jsons[i]['body']['metadata_t'])
            ...
            if bottom_pad != 0:
                log.info(f"Unpadding {bottom_pad} pixels from the bottom of the images.")
