...
class KlothedBodyV3(Callable):
    ...
    def calculate_ipd_based_height(self,
        ...
        log.info(f"Estimated ({gender}) Mesh IPD = {float(ipd):.4f}cm, Mesh Scaled Height = {float(height):.4f}m, Actual Height = {float(actual_height):.4f}")
        return (
    ...
    def save_body_legacy(self, 
        ...
        for i in range(len(jsons)):
            ...
            gender = 'neutral' if gender_id is None else toolz.get(gender_id.item(), KlothedBodyV3._GENDERS_, 'neutral')
    ...
    def __call__(self, 
        ...
        for i in range(b):
            ...
            gender = openpose_data['people'][0].get('gender', 'neutral')
            data['gender'] = torch.scalar_tensor(KlothedBodyV3.__GENDER_IDS__[gender]).int()
            if dy != 0.0:
