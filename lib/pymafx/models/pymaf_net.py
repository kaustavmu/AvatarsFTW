...
from lib.pymafx.utils.geometry import rot6d_to_rotmat, rotmat_to_rot6d, projection, rotation_matrix_to_angle_axis, rotmat_to_angle, compute_twist_rotation
...
class PyMAF(nn.Module):
    ...
    def forward(self, input_batch={}, J_regressor=None, rw_cam={}):
        ...
        for rf_i in range(cfg.MODEL.PyMAF.N_ITER):
            ...
            # just to work with main pipeline
            out_list['smpl_out'] = out_list['mesh_out']
        return out_list, vis_feat_list

def pymaf_net(smpl_mean_params, device=torch.device('cuda'), pretrained=True):
    """ Constructs an PyMAF model with ResNet50 backbone.
    Args:
        is_train (bool): If True, load ImageNet/COCO pre-trained models
    """
    is_train = not pretrained
    model = PyMAF(smpl_mean_params, device, is_train=is_train)
    return model
